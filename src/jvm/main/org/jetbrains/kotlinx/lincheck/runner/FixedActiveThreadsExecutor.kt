...
internal class FixedActiveThreadsExecutor(private val nThreads: Int, runnerHash: Int) : Closeable {
    ...
    private fun submitTask(iThread: Int, task: Any) {
        while (true) {
            val value = tasks[iThread].value
            if (tasks[iThread].compareAndSet(value, task)) {
                if (value is TestThread) {
    ...
    class TestThread(val iThread: Int, val runnerHash: Int, r: Runnable) : Thread(r, "Lincheck-RunnerThread-$iThread") {
